...
class LLaVAClient:
    def __init__(self, base_url='http://localhost:6800'):
        ...
        # The cluster exports an HTTP proxy that may intercept even localhost.
        # Rollout inference is always a local service, so bypass proxy settings.
    ...
    def process_frame(self, text, episode_first_frame, **kwargs):
        ...
        if "response" not in payload:
            raise RuntimeError(f"Inference response is missing 'response': {payload}")
