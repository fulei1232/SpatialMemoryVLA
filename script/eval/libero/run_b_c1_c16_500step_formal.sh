...
output_root="${OUTPUT_ROOT:-${storage_root}/evaluations/libero_spatial_occ_b_c1_c16_500step_formal}"
trials="${NUM_TRIALS_PER_TASK:-50}"
inference_seed="${INFERENCE_SEED:-7}"

labels=(B C1 C16)
gpus=(0 1 2)
ports=(27820 27821 27822)
checkpoints=(
...
run_model() {
  ...
  for condition in normal temporal_occlusion; do
    for task_id in 0 8; do
      local task_dir="${model_root}/${condition}/task-${task_id}"
      mkdir -p "${task_dir}"
      env MUJOCO_GL=egl MUJOCO_EGL_DEVICE_ID="${gpu}" CUDA_VISIBLE_DEVICES="${gpu}" \
        .venv/bin/python evaluation/libero/eval_libero.py \
          ...
          --run_id_note "${label}-${condition}-task${task_id}" \
...
echo "[$(date --iso-8601=seconds)] formal evaluation start: ${trials} states/task" > "${output_root}/progress.log"
for i in "${!labels[@]}"; do
...
echo "[$(date --iso-8601=seconds)] formal evaluation complete" >> "${output_root}/progress.log"
