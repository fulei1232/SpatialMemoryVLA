...
output_root="${OUTPUT_ROOT:-${storage_root}/evaluations/libero_spatial_occ_b_c1_c16_500step_long16}"
trials="${NUM_TRIALS_PER_TASK:-50}"
inference_seed="${INFERENCE_SEED:-7}"

labels=(B C1 C16)
gpus=(0 1 2)
ports=(27830 27831 27832)
checkpoints=(
...
run_model() {
  ...
  for task_id in 0 8; do
    local task_dir="${model_root}/temporal_occlusion/task-${task_id}"
    mkdir -p "${task_dir}"
    env MUJOCO_GL=egl MUJOCO_EGL_DEVICE_ID="${gpu}" CUDA_VISIBLE_DEVICES="${gpu}" \
      .venv/bin/python evaluation/libero/eval_libero.py \
        ...
        --occlusion_schedule_steps 32 \
        --occlusion_partial_start_ratio 0.15 \
        --occlusion_full_start_ratio 0.25 \
        --occlusion_recovery_start_ratio 0.75 \
        --save_rollout_videos False \
        --run_id_note "${label}-long16-task${task_id}" \
...
echo "[$(date --iso-8601=seconds)] long-occlusion evaluation start: visible=5, partial=3, full=16 calls; ${trials} states/task" > "${output_root}/progress.log"
for i in "${!labels[@]}"; do
...
echo "[$(date --iso-8601=seconds)] long-occlusion evaluation complete" >> "${output_root}/progress.log"
