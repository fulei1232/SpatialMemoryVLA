...
from spatial_teacher import SpatialTeacher
...
class TrainConfig:
    ...
    experiment_mode: str = "memoryvla" # Matched ablation label: memoryvla | spatial_forcing | spatial_memory
    ...
    def __post_init__(self) -> None:
        ...
        # Draccus accepts a scalar CLI override for a tuple field without
        # coercing it.  Normalize the common `--trackers jsonl` form so metrics
        # does not iterate over the characters in "jsonl".
        if isinstance(self.trackers, str):
            self.trackers = (self.trackers,)
        elif isinstance(self.trackers, tuple) and self.trackers and all(
            isinstance(tracker, str) and len(tracker) == 1 for tracker in self.trackers
        ):
            # A scalar override for Tuple[str, ...] is decoded by Draccus as a
            # tuple of characters before __post_init__ runs.
        ...
        expected_spatial_flags = {
            "memoryvla": (False, False),
            "spatial_forcing": (True, False),
            "spatial_memory": (True, True),
        }
        if self.experiment_mode not in expected_spatial_flags:
            raise ValueError(
                f"Unknown experiment_mode={self.experiment_mode!r}; expected one of "
                f"{tuple(expected_spatial_flags)}"
            )
        actual_flags = (self.use_spatial_forcing, self.use_spatial_memory)
        if actual_flags != expected_spatial_flags[self.experiment_mode]:
            raise ValueError(
                f"experiment_mode={self.experiment_mode!r} requires spatial flags "
                f"{expected_spatial_flags[self.experiment_mode]}, got {actual_flags}"
            )
...
def train(cfg: TrainConfig) -> None:
    ...
    spatial_teacher = None
    if cfg.use_spatial_forcing:
        if cfg.spatial_teacher_path is None:
            raise ValueError("spatial_teacher_path is required when use_spatial_forcing=True")
        overwatch.info(f"Loading frozen VGGT spatial teacher from `{cfg.spatial_teacher_path}`")
        spatial_teacher = SpatialTeacher.from_checkpoint(
            cfg.spatial_teacher_path,
            feature_layer=cfg.spatial_teacher_feature_layer,
            target_tokens=vla._num_patches(),
        ).to(device_id)
        spatial_teacher.eval()
    ...
    overwatch.info(
        ...
        f"         |-> Pretrained Checkpoint = {cfg.pretrained_checkpoint}\n"
        f"         |-> Dataset Mix = {cfg.vla.data_mix}\n"
        f"         |-> Experiment Mode = {cfg.experiment_mode}\n"
        f"         |-> Trainable Parameter Count = {num_trainable_params}\n"
    ...
    train_strategy = get_train_strategy(
        ...
        spatial_teacher=spatial_teacher,
    )
