...
def get_dataset_statistics(
    ...
    # Check if a cache file exists and load it.  Dataset initialization occurs in
    # every distributed rank, so cache publication below must be atomic: a rank
    # must never observe another rank's partially-written JSON.
    ...
    try:
        ...
        # For local filesystems rename is atomic.  Multiple ranks may still do
        # duplicate one-time work, but readers will only ever see a complete
        # cache file.
        ...
        local_tmp_path = f"{local_path}.tmp-{os.getpid()}"
