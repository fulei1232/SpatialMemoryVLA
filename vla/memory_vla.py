...
from vla.spatial_alignment import SpatialAlignProjector, extract_visual_hidden
...
class MemoryVLA(nn.Module):
    def __init__(
        ...
        spatial_align_layer: int = 24,
        spatial_align_coeff: float = 0.5,
        ...
        **kwargs,
        ...
        self.consolidate_type = consolidate_type
        self.update_fused = update_fused
        self.use_spatial_forcing = use_spatial_forcing
        self.use_spatial_memory = use_spatial_memory
        self.spatial_align_layer = spatial_align_layer
        self.spatial_align_coeff = spatial_align_coeff
        ...
        if self.use_spatial_forcing or self.use_spatial_memory:
            self.align_projector = SpatialAlignProjector(
                llm_dim=self.cog_token_size,
                teacher_dim=self.spatial_teacher_dim,
                use_vlm_norm=use_vlm_norm,
            )
        if self.use_spatial_memory:
            self.spatial_compr = BottleneckSE(
                C_in=self.spatial_teacher_dim,
                C_mid=self.per_token_size * 2,
                C_out=self.per_token_size,
            )
    ...
    def _num_patches(self) -> int:
        if self.vlm.vision_backbone.featurizer is not None:
            return self.vlm.vision_backbone.featurizer.patch_embed.num_patches
        if hasattr(self.vlm.vision_backbone, 'siglip_featurizer') and self.vlm.vision_backbone.siglip_featurizer is not None:
            return self.vlm.vision_backbone.siglip_featurizer.patch_embed.num_patches
        raise ValueError("No vision backbone found")

    def _build_perception_tokens(self, output, num_patch: int, spatial_target: Optional[torch.Tensor] = None):
        """Build raw or geometry-aligned perception tokens and optional auxiliary loss."""
        if not (self.use_spatial_forcing or self.use_spatial_memory):
            return self.per_compr(self.vlm.vision_feats), None
        if self.use_spatial_forcing and spatial_target is None:
            raise ValueError("spatial_target is required when use_spatial_forcing=True")
        vision_hidden = extract_visual_hidden(output.hidden_states, self.spatial_align_layer, num_patch)
        spatial_feat = self.align_projector.align_dimension(vision_hidden)
        spatial_loss = self.align_projector.alignment_loss(spatial_feat, spatial_target) if spatial_target is not None else None
        if self.use_spatial_memory:
            per_tokens = self.spatial_compr(spatial_feat)
        else:
            per_tokens = self.per_compr(self.vlm.vision_feats)
        self._assert_spatial_contract(vision_hidden, spatial_feat, spatial_target, per_tokens)
        return per_tokens, spatial_loss

    def _assert_spatial_contract(
        self,
        vision_hidden: torch.Tensor,
        spatial_feat: torch.Tensor,
        spatial_target: Optional[torch.Tensor],
        per_tokens: torch.Tensor,
    ) -> None:
        """Fail early on a changed token layout instead of silently training the wrong graph."""
        if not self.spatial_debug_asserts:
            return
        expected_patch_shape = (256, self.cog_token_size)
        expected_spatial_shape = (256, self.spatial_teacher_dim)
        expected_per_shape = (256, self.per_token_size)
        assert tuple(vision_hidden.shape[-2:]) == expected_patch_shape, (
            f"VLA layer-{self.spatial_align_layer} visual hidden must be [B,{expected_patch_shape[0]},"
            f"{expected_patch_shape[1]}], got {tuple(vision_hidden.shape)}"
        )
        assert tuple(spatial_feat.shape[-2:]) == expected_spatial_shape, (
            f"Aligned feature must be [B,{expected_spatial_shape[0]},{expected_spatial_shape[1]}], "
            f"got {tuple(spatial_feat.shape)}"
        )
        if spatial_target is not None:
            assert tuple(spatial_target.shape[-2:]) == expected_spatial_shape, (
                f"VGGT target must be [B,{expected_spatial_shape[0]},{expected_spatial_shape[1]}], "
                f"got {tuple(spatial_target.shape)}"
            )
            assert torch.isfinite(spatial_target).all(), "VGGT spatial target contains NaN or Inf"
        assert tuple(per_tokens.shape[-2:]) == expected_per_shape, (
            f"Perception tokens must be [B,{expected_per_shape[0]},{expected_per_shape[1]}], "
            f"got {tuple(per_tokens.shape)}"
        )
        assert torch.isfinite(spatial_feat).all(), "Aligned spatial feature contains NaN or Inf"
        if not self._spatial_contract_logged:
            overwatch.info(
                "Spatial contract verified: VLA [B,256,4096] -> align [B,256,2048] "
                "-> perception memory [B,256,256]"
            )
            self._spatial_contract_logged = True
    ...
    def forward(
        ...
        output = self.vlm(
            ...
            # Spatial alignment consumes an intermediate LLM layer even if a
            # caller omitted this legacy flag.
            output_hidden_states=(output_hidden_states or self.use_spatial_forcing or self.use_spatial_memory),
            return_dict=return_dict,
        )

        num_patch = self._num_patches()
        ...
        per_tokens, spatial_loss = self._build_perception_tokens(output, num_patch, spatial_target)
        ...
        action_loss = self.action_model.loss(
            actions_repeated,
            cog_tokens_repeated,
            per_tokens_repeated,
        )
        spatial_weighted_loss = torch.zeros_like(action_loss)
        if spatial_loss is not None:
            spatial_weighted_loss = self.spatial_align_coeff * spatial_loss
        loss = action_loss + spatial_weighted_loss
        if self.spatial_debug_asserts:
            assert torch.isfinite(action_loss), "Diffusion action loss is NaN or Inf"
            if spatial_loss is not None:
                assert torch.isfinite(spatial_loss), "Spatial alignment loss is NaN or Inf"
            assert torch.isfinite(loss), "Total loss is NaN or Inf"

        # The external teacher remains outside FSDP and checkpoint state.
        # Metrics are detached diagnostics, not part of the training graph.
        spatial_metrics = {
    ...
    def predict_action(
        ...
        **kwargs: str
        ...
        if self.use_spatial_memory:
            # Generation stores a tuple of per-step hidden-state tuples.  The
            # first step contains the full prompt including visual tokens.
            generation_hidden = output.hidden_states[0] if isinstance(output.hidden_states[0], tuple) else output.hidden_states
            vision_hidden = extract_visual_hidden(generation_hidden, self.spatial_align_layer, self._num_patches())
            per_tokens = self.spatial_compr(self.align_projector.align_dimension(vision_hidden))
        else:
            per_tokens = self.per_compr(self.vlm.vision_feats)
