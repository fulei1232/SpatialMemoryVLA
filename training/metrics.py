        ...
        for key, value in kwargs.items():
            if key == "loss":
                ...
                if key not in self.state:
                    self.state[key] = deque(maxlen=self.state["loss"].maxlen)
...
class VLAMetrics:
        ...
        **kwargs,
        ...
        for key, value in kwargs.items():
            if key == "loss":
                ...
                # Auxiliary losses and diagnostics are supplied by model
                # extensions (for example spatial_loss and grad_norm).  Keep
                # VLAMetrics extensible instead of requiring every caller to
                # modify this constructor whenever it adds a scalar metric.
                if key not in self.state:
                    self.state[key] = deque(maxlen=self.state["loss"].maxlen)
                self.state[key].append(value.detach())
    ...
    def push(self) -> str:
        ...
        logged_metrics = {
            ...
            f"{prefix}/Loss": loss,
            f"{prefix}/Total Loss": loss,
            f"{prefix}/Loss (Raw)": loss_raw,
            f"{prefix}/Learning Rate": lr,
            f"{prefix}/Step Time": step_time,
        }
        for key, values in self.state.items():
            if key in {"loss_raw", "loss", "l1_loss", "action_accuracy", "step_time", "lr"} or not values:
                continue
            metric_name = key.replace("_", " ").title()
            logged_metrics[f"{prefix}/{metric_name}"] = torch.stack(list(values)).mean().item()
        self.log(
            self.global_step,
