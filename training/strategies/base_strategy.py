...
class TrainingStrategy(ABC):
    def __init__(
        ...
        spatial_teacher: Optional[torch.nn.Module] = None,
        **_: str,
    ) -> None:
        ...
        # This module is deliberately external to the FSDP-wrapped VLA.  It
        # supplies no-grad targets and is never checkpointed with the policy.
        self.spatial_teacher = spatial_teacher
        if self.spatial_teacher is not None:
            self.spatial_teacher.requires_grad_(False)
            self.spatial_teacher.eval()
            assert not any(parameter.requires_grad for parameter in self.spatial_teacher.parameters())
        ...
        with tqdm(
            ...
            for train_idx, batch in enumerate(dataloader):
                ...
                with torch.autocast(
                    ...
                    if self.spatial_teacher is not None:
                        with torch.no_grad():
                            spatial_target = self.spatial_teacher(batch["pixel_values"])
                    loss, output, spatial_metrics = self.vlm(
                ...
                if not torch.isfinite(loss):
                    raise FloatingPointError(f"Non-finite total loss at micro-step {train_idx}: {loss.detach()}")
                # Commit Loss =>> Backward!
                metrics.commit(loss=loss, **spatial_metrics)
                ...
                if (train_idx + 1) % self.grad_accumulation_steps == 0:
                    ...
                    if not torch.isfinite(grad_norm):
                        raise FloatingPointError(
                            f"Non-finite gradient norm before optimizer step {metrics.global_step + 1}: "
                            f"{grad_norm.detach()}"
                        )
