...
class FSDPStrategy(TrainingStrategy):
    def __init__(
        ...
        spatial_teacher: Optional[nn.Module] = None,
    ...
    def save_checkpoint(
        ...
        """Save model and optimizer state needed for a real FSDP resume."""
        ...
        with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy, self.fsdp_save_optimizer_policy):
            ...
            full_optimizer_state_dict = FSDP.optim_state_dict(self.vlm, self.optimizer)
            model_state_dicts = {
            ...
            if overwatch.is_rank_zero():
                ...
                self._save_executor.submit(torch.save, {"optimizer": full_optimizer_state_dict}, optimizer_path)
                with (run_dir / "checkpoint-events.jsonl").open("a") as checkpoint_log:
                ...
                overwatch.info(f"Saving model checkpoint to {checkpoint_path} and optimizer state to {optimizer_path}")
